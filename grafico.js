function Grafico(parametros) {
   ...
   svg
    ...
    .attr('y', 0)
    .style('text-anchor', 'middle')
    .attr('transform', 'translate(30, ' + (margem.superior + this.alturaPlotagem / 2) + 
    ') rotate(-90)'
    )
    .text(parametros.tituloY);

    this.atualize = (dados) => {
        this.fnX.domain(dados.map((d) => d.chave));
        this.fnY.domain([0, d3.max(dados.map((d) => d.valor))]);
